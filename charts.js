...
function buildCharts(sample) {
  ...
  d3.json("data/samples.json").then((data) => {
    ...
    var samplesArray = dataSamples.filter(sampleObj => sampleObj.id == sample);
    ...
    var trace1 = {
      x: xticks.reverse(),
      y: yticks.map(ID => `OTU ${ID}`).reverse(),
      type: 'bar',
      orientation: 'h',
    ...
    // GAUGE CHART CODE
    // 1. Create a variable that filters metadata array for the object with the desired sample number.
    var metadata = data.metadata;
    var metadataArray = metadata.filter(metaObj => metaObj.id == sample);
    var metadataFirstSample = metadataArray[0];
    console.log(metadataFirstSample);
    var washFrequency = +(metadataFirstSample.wfreq);
